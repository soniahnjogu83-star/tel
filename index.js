  ...
  try {
    ...
    if (code === 0) {
      ...
      safeSendMessage(id,
        ...
        `📋 *If your M-Pesa was actually deducted*, please type your *M-Pesa confirmation code* from your SMS (e.g. \`RCX4B2K9QP\`) and it will be verified automatically. 🔍\n\n` +
...
bot.on("message", async (msg) => {
  ...
  // ── Handle M-Pesa receipt code submitted by user (in awaitingReceipt state) ──
  if (awaitingReceipt[chatId]) {
    ...
    const finalPlan = receiptInfo.plan || (userSelections[chatId] || {}).plan || "1 Month";

    // Update userSelections so the plan is marked as paid
    const selNow = userSelections[chatId] || {};
    selNow.paidAt = new Date().toISOString();
    selNow.stkRef = code;
    if (!selNow.plan    && receiptInfo.plan)    selNow.plan    = receiptInfo.plan;
    if (!selNow.package && receiptInfo.pkg)     selNow.package = receiptInfo.pkg;
    userSelections[chatId] = selNow;
    saveUserSelection(chatId, selNow);
    clearReminders(chatId);

    recordPayment({
      chatId,
      username: selNow.username || chatId,
      pkg:      receiptInfo.pkg  || selNow.package || "N/A",
      plan:     finalPlan,
      amount:   receiptInfo.price || selNow.price || 0,
      ref:      code,
      phone:    "manual",
    });

    // Notify admins (informational — access is already being sent)
    notifyAdmins(
      `🔔 *Manual Receipt — Auto-Approved*\n\n` +
      `👤 ChatID: \`${chatId}\`\n` +
      `📦 ${receiptInfo.pkg || selNow.package || "N/A"} — ${finalPlan}\n` +
      `💰 Ksh ${receiptInfo.price || selNow.price || "N/A"}\n` +
      `🧾 M-Pesa Code: \`${code}\`\n\n` +
      `✅ Access granted automatically. Use /kick ${chatId} if this was fraudulent.`
    );

    return grantAccess(
      chatId,
      finalPlan,
      `✅ Ksh *${receiptInfo.price || selNow.price || "?"}* — M-Pesa Code: \`${code}\``
    );
  }

  // ── User typed something freely — detect a 10-char M-Pesa code ────────────
  ...
  if (looksLikeCode) {
    ...
    // ── AUTO-APPROVE free-text code ────────────────────────────────────────
    const finalPlan = sel2.plan || "1 Month";
    ...
    notifyAdmins(
      `🔔 *Receipt Code — Auto-Approved*\n\n` +
      `👤 ChatID: \`${chatId}\`\n` +
      `📦 ${sel2.package || "N/A"} — ${finalPlan}\n` +
      `💰 Ksh ${sel2.price || "N/A"}\n` +
      `🧾 M-Pesa Code: \`${code}\`\n\n` +
      `✅ Access granted automatically. Use /kick ${chatId} if this was fraudulent.`
    );
    ...
    return grantAccess(
      chatId,
      finalPlan,
      `✅ Ksh *${sel2.price || "?"}* — M-Pesa Code: \`${code}\``
    );
  }
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "confirm_payment") {
    ...
    return safeSendMessage(chatId,
      ...
      `Once you send it, your access will be granted *immediately and automatically*. 🔍`,
      { parse_mode: "Markdown" }
    );
  }
...
function getManualPaymentMessage(sel) {
  return {
    text: tillCard(sel.package, sel.plan, sel.price) +
      `\n\n✅ *Once you have paid:*\n1. Tap the button below\n2. Send your *M-Pesa Confirmation Code* (e.g. RCX4B2K9QP)\n\n_Your access will be granted automatically the moment you send the code._ ⚡`,
